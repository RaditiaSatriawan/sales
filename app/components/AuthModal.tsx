...
export default function AuthModal({
			...
			if (isLogin) {
				...
				if (response.success) {
					...
					if (invoiceResponse.success && invoiceResponse.token) {
						...
						setError(
							invoiceResponse.error || "Gagal mendapatkan token pembayaran",
						);
					}
				} else {
				...
				if (response.success) {
					...
					if (loginResponse.success) {
						...
						setError(
							loginResponse.error ||
								"Login otomatis gagal, silakan login manual",
						);
					}
				} else {
					setError(response.error || "Terjadi kesalahan, coba lagi");
	...
	const handleGoogleAuth = async () => {
		...
		try {
			...
			// loginWithGoogle redirects the page — code below will not run
		} catch (err) {
